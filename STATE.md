...
- 🎻 **ใหม่ 16 มิ.ย.: เสียงวงจริง BBCSO + headless render pipeline** — ติดตั้ง BBC Symphony Orchestra Discover + REAPER · `clips/_mid2rpp.py` แปลง MIDI→เสียงวงจริง **ผ่าน command line ไม่เปิดจอ** (`reaper.exe -renderproject` · รักษา timing) · ทดสอบ violin/percussion ✓ · รายงานเต็ม `G:\My Drive\horatad-report\BGM_BBCSO_MASTERPLAN_REPORT.html` · MIDI คลัง `clips/mozart`,`clips/beethoven` + `G:\My Drive\horatad-midi\`
  - **⏳ ค้าง (18 มิ.ย.): ฟัง BBCSO ครบทุกเครื่อง (palette audition)** — เลือก scope "ฟังครบทุกเครื่อง" · พี่ตั้ง REAPER เอง · **ผมเตรียมแล้ว:** MIDI ทดสอบกลาง `clips/_bbcso_palette_test.mid` (gen ด้วย `_make_palette_test.py`) + สคริปต์ render `clips/_render_palette.py` (solo ทีละ track → `reaper -renderproject` → WAV ต่อเครื่องใน `_palette_out/`) · **รอพี่:** ทำ multitrack (แต่ละ track = BBCSO 1 เครื่อง + วาง MIDI ทดสอบเดียวกัน) save เป็น `clips/_bbcso_palette.rpp` → ผมรัน `python _render_palette.py` + ตัดเข้า `_review/` ให้ฟังเทียบ
