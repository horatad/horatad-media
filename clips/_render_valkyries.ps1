...
foreach ($it in $items) {
  ...
  $mp3 = "C:\horatad-media\clips\Valkyries - $($it.lab).mp3"
  $wav = "C:\horatad-media\clips\_vktmp.wav"
  Write-Output ("rendering " + $it.lab + " ...")
  & $fs -ni -o synth.polyphony=1024 -r 44100 -g 0.9 -F $wav $sf $mid 2>$null | Out-Null
  if (Test-Path $wav) {
